...
class UserEnrollment:
    ...
    def reEnrollDatabase(self):
        ...
        for personName in os.listdir(self.faceImageOutputPath):
            for imageName in os.listdir(f"{self.faceImageOutputPath}/{personName}/images"):
                ...
                # reenroll without saving image again
                self.enrollImage(personName, image, saveData=False)
    ...
    def enrollVoice(self, name : str , audio : np.ndarray, saveData : bool=True) -> np.ndarray:
        '''
        ...
        Inputs:
            ...
            optional
                saveData (bool) - defaults to True, saves data processed alongside the features extracted from data
        Outputs:
        ...
        # audio preprocessing
        audio = self.speakerRecognition.preprocessAudio(audio)
        ...
        num = countFilesInDir(featuresSavePath) # get number of current features
        filename = f"{name}-{num+1}" # save new image with a number of current images + 1
        T = (name, embeddings)
    ...
    def enrollImage(self, name : str, image : np.ndarray, saveData : bool=True) -> np.ndarray:
        '''
        ...
        Inputs:
            ...
            optional
                saveData (bool) - defaults True, save original data from which features were extracted from
        ...
        # pickle extracted features
        num = countFilesInDir(featuresSavePath)
        filename = f"{name}-{num+1}" 
        T = (name, sift_test_descriptors, cnn_embeddings, vgg_embeddings)
        self.pickler.save_to(f"{featuresSavePath}/{filename}.pkl", T)
